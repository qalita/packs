...
table names with parquet paths through ``zip``, which on a chunked source
either dropped parts 2..N or promoted each part to its own dataset. Both
produce a plausible-looking score computed on a fraction of the rows.
"""
...
def test_view_survives_a_reopened_database(tmp_path, parts):
    """GX reconnects through SQLAlchemy, so the views must be persisted."""
