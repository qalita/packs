...
with Pack() as pack:
    ...
    for dataset_name, df_curr in items:
        ...
        profile = ProfileReport(
            df_curr, minimal=True, title=f"Profiling Report for {dataset_name}"
        )
        ...
        with open(
            f"{dataset_name}_report.json", "r", encoding="utf-8"
        ) as file:
        ...
        column_list_hash = hashlib.md5(
            column_list_str.encode(), usedforsecurity=False
        ).hexdigest()
        pack.metrics.data.append(
            {
        ...
        column_order_hash = hashlib.md5(
            column_order_str.encode(), usedforsecurity=False
        ).hexdigest()
        pack.metrics.data.append(
            {
        ...
        types_str = ",".join(
            [f"{col}:{df_curr[col].dtype}" for col in df_curr.columns]
        )
        column_types_hash = hashlib.md5(
            types_str.encode(), usedforsecurity=False
        ).hexdigest()
        pack.metrics.data.append(
            {
        ...
        print(
            f"Schema metrics: {column_count} columns, list_hash={column_list_hash[:8]}..., types_hash={column_types_hash[:8]}..."
        )
